...
def run_system(user_data):
    ...
    # If parsing failed, use fallback markers
    if not treatment:
        parts = re.split(r"TREATMENT", full_output, flags=re.IGNORECASE)
        if len(parts) > 1:
            treatment = parts[1].split("HOSPITAL")[0].strip("#[]:* \n\t")
    
    if not hospitals:
        parts = re.split(r"HOSPITAL", full_output, flags=re.IGNORECASE)
        if len(parts) > 1:
            hospitals = parts[1].split("COST")[0].strip("#[]:* \n\t")
            
    if not costs:
        parts = re.split(r"COST", full_output, flags=re.IGNORECASE)
        if len(parts) > 1:
            costs = parts[1].strip("#[]:* \n\t")

    # If still empty, use reasonable defaults or empty strings instead of full_output
    if not treatment and full_output:
        treatment = "Treatment plan could not be parsed. Please check raw output."
    if not hospitals and full_output:
        hospitals = "Hospital list could not be parsed."
    if not costs and full_output:
        costs = "Cost breakdown could not be parsed."
